...
After route dispatch, the gateway SHALL:
1. Remove the `Proxy-Authorization` header from the outgoing request
2. Inject `Authorization: Bearer <upstream-key>` from the matched route's credential env var
3. Return `503 Service Unavailable` if the required credential env var is absent or empty
