...
Prove the gateway binary works end-to-end (phantom token, routing, credential injection)
without any TEE-specific dependencies. Runs locally via `docker run` or on any plain VM.
The `/attest` endpoint is not present yet — this phase is purely about the proxy data plane.
...
- [ ] 4.1 Write a multi-stage `Dockerfile`: `cargo-chef` prepare + cook stage, `cargo build --release` stage, minimal runtime stage (`debian:bookworm-slim` with CA certificates). The build fetches `nono-proxy` via Cargo git dep — no submodule checkout needed
...
- [ ] 5.1 Run locally with `docker compose up` and dummy env vars; confirm the server binds and returns `407` on a bare `curl localhost:8080/openai/`
- [ ] 5.2 Set real `COCO_PHANTOM_TOKEN` and `OPENAI_API_KEY`; send an OpenAI chat completion request via the gateway and confirm a valid response is received
- [ ] 5.3 Confirm that `OPENAI_API_KEY` never appears in any outbound request from the host (check via `tcpdump` or a local mitmproxy)
...
Take the proven proxy binary from Phase 1a and promote it to a Phala Cloud TDX CVM.
Add the `/attest` endpoint, Phala secret injection, and CI publishing.

### 6. Attestation Endpoint

- [ ] 6.1 Add `reqwest` dependency for tappd HTTP call
- [ ] 6.2 Implement `GET /attest` handler: call Phala tappd at `http://localhost:8090/prpc/Tappd.TdxQuote` with a Unix-timestamp nonce in `report_data`; decode base64 response, hex-encode, return JSON `{ "quote": "<hex>", "platform": "tdx", "debug": <bool> }`
- [ ] 6.3 Parse the raw TDX quote bytes to inspect `td_attributes` bit 0; log `ERROR: TDX debug mode detected` to stderr when set and include `"debug": true` in the response
- [ ] 6.4 Return `503` with an explanatory message from `GET /attest` when tappd is unreachable (local dev / non-Phala environment)
- [ ] 6.5 Wire `GET /attest` into the Axum router (no auth required on this route)
...
- [ ] 9.1 Update README.md to document Phase 1a (plain VM / local) and Phase 1b (CVM) as the two-step POC path
- [ ] 9.2 Add egress enforcement gap section: describe the threat model weakening, mitigation options (egress firewall, Path C), and Path B vs Path C comparison table
- [ ] 9.3 Document deployment steps in `DEPLOY.md`: provision secrets via `phala cvms secrets set`, push image to GHCR, deploy via Phala dashboard or CLI

### 10. CVM Validation

- [ ] 10.1 Call `GET /attest` on the deployed gateway and confirm the response contains a hex-encoded TDX DCAP QuoteV4 with `"platform": "tdx"` and no debug flag
- [ ] 10.2 Run end-to-end test: configure an OpenAI Python client with `base_url=https://<cvm-host>/openai/v1` and phantom token, send a chat completion request, confirm a valid response is received
- [ ] 10.3 Confirm via network capture or logging that `OPENAI_API_KEY` never appears in the agent process or outbound request from the agent host
